...
if __name__ == "__main__":
    try:
        ...
        lb_steer = SteerMotor('lb', 13) # init steer motor class
        period = 0.05 # used for PID controller
        rate = rospy.Rate(20)
        while not rospy.is_shutdown():
            start = rospy.get_time()
            #lb_steer.adjust_max_speed()
            lb_steer.adjust_speed(period) # run pid controller
            rate.sleep()
            end = rospy.get_time()
            period = end - start
